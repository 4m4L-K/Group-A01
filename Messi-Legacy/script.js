...
/* ── FAN VIDEO — scroll-triggered autoplay (fan zone) ───────── */
(function initFanVideo() {
  const video  = document.getElementById('fanvideo');
  const frame  = document.getElementById('fanvideo-frame');
  if (!video) return;
  ...
  const obs = new IntersectionObserver(entries => {
    entries.forEach(e => {
      if (e.isIntersecting && !hasPlayed) {
        ...
        if (frame) frame.classList.add('video-playing');
...
function toggleFanVideoMute() {
  const video   = document.getElementById('fanvideo');
  const label   = document.getElementById('fanvideo-mute-label');
